...
path = os.getcwd()
output = path + "/staticfiles/result/result_img.png"

# 플레이리스트 타이틀 번역 및 전처리
def title2prompt(new_title):
    translator=Translator()
    new_title_trans=translator.translate(new_title,src='ko',dest='en').text
    prompt_str = 'photograph, ' + new_title_trans + ', landscape, 8k uhd'
    return prompt_str
    

def generateImg(prompt_str):
    print(f"generateImg_str: {prompt_str}, type: {type(prompt_str)}")
