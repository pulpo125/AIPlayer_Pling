...
def DicGenerator(train, song_meta):
    ...
    for i in range(len(song_meta)):
        song_issue_dic[song_meta[i]['id']] = song_meta[i]['issue_date'] # id = 발매일
...
def Recommender(train, questions, n_msp, n_mtp, mode, sim_measure, song_meta, freq_song, save=False): # n_msp=50
    ## 최종 추천리스트
    rec_list = []

    ## 1단계: 전처리
    # 1) 추천 결과가 없거나 모자란 경우를 위해 most_popular 생성
    _, song_mp = most_popular(train, "songs", 200) # 빈도수  top 200개
    _, tag_mp = most_popular(train, "tags", 20) # 빈도수 Top 20개
